...
##ortak sınıf oluştur. onceki ekrana gitmede ki tekrarı engellemek için
...
class GirisPenceresi(BaseDialog, GirisFormu):
    ...
    def giris_basarili_yonlendir(self, rol, kullanici_data):
                                               #bu kısım kuaför ise kuaför panaline, müşteri ise müşteri panaline yönlendirilecektir
        self.close()
...
class MusteriKayitPenceresi(BaseDialog, MusteriKayitForm):
    ...
    def musteri_kaydi_yap(self):
        ...
        if self.db_manager:     ##db deki musteri_ekle metodu çağırlır
    ...
    def kuafor_kaydi_yap(self):
        ...
        if self.db_manager:
            ...
            if inserted_id:
                ...
                #başarılı kayıt sonrası kayıt seçim ekranı kapatılıp
                #giriş ekranına geri dönülmesini sağlayan metot çağırılır
                self.accept()
                self.geri_don_ana_ekrana()
...
class KuaforAnaPenceresi(QMainWindow, KuaforAnaEkran):
    def __init__(self, kullanici_data, db_manager):
        ...
        if hasattr(self, "bugun_pushButton"):
            self.bugun_pushButton.clicked.connect(self.bugune_git)
                                                                         ## HASATTR IN İÇİ
        if hasattr(self, "takvimK_calendarWidget"):    ####### QTDESİGNER DAKİ İSİMLERLE AYNI OLMALILAR#####    
    ...
    def bugune_git(self):
        #takvimmi bugüne odaklar
        self.takvimK_calendarWidget.setSelectedDate(QtCore.QDate.currentDate())



    def randevulari_yukle(self):  #veritabanından veriyi çeken ana fonk
        try:

            #kuafor id sini ve takvimden seçilen tarihi al
            kuafor_id = str(self.kullanici_data["_id"])
            secili_tarih = self.takvimK_calendarWidget.selectedDate().toString("dd-MM-yyyy")


            #veritabanından bu kuaföre ait tarihteki randevuları çek
    ...
    def randevulari_listele(self, randevular, secili_tarih):
        try:
            ...
            # Yeni eklediğin listWidget'ı temizle
...
class KuaforProfilPenceresi(QMainWindow, KuaforProfil):
    def __init__(self, kullanici_data, db_manager, ana_ekran_referansi):
        ...
        self.kullanici_data = kullanici_data
        self.db_manager = db_manager
        self.ana_ekran_referansi = ana_ekran_referansi    #geri dönmke için lazım

        self.bilgileri_yukle()

        #bağlantıları burada tanımlıyoruz
    ...
    def kuafor_bilgilerini_kaydet(self):
        ...
        try:
            ...
            self.kullanici_data.update(yeni_veri)    #yerel veriyi güncelle
    ...
    def hesabimi_sil(self):
        ...
        if onay == QtWidgets.QMessageBox.Yes:
            try:
                #veritabanından silme işlmei gerçekleşir
    ...
    def ana_ekrana_don(self):           #prfil ekranını kapatır ve ana panele geçer
        self.close()
...
class MusteriAnaPenceresi(QMainWindow, MusteriAnaEkran):
    def __init__(self, kullanici_data, db_manager):
        ...
        #ilçeleri doldurmak içindir bu satır
        self.ilceleri_yukle()   ####yeni ekledi
    ...
    def ilceleri_yukle(self):
        #Kütahya ilçelerini combobox a yazar
        ilceler = ["Merkez", "Altıntaş" ,"Domaniç", "Emet", "Gediz", "Hisarcık", "Simav", "Tavşanlı"]
        #tasrımdaki combox ın adını yazarız ilcelerinki
        if hasattr(self, "ilce_comboBox"):
            ...
            #ilçe değiştiğinde kuaförleri getiren fonksiyonu bağla
            self.ilce_comboBox.currentIndexChanged.connect(self.kuaforleri_filtrele)



    def kuaforleri_filtrele(self):
        #seçilen ilçeye göre veritabanından kuaförleri çeker
        ...
        try:
            #database de ki query metodunu kullanıyoruz
    ...
    def randevu_onayla(self):
        try:
            ...
            # Eksik bilgi kontrolü (Kuaför seçilmemişse)
            if not randevu_verisi["kuafor_id"]:
                QtWidgets.QMessageBox.warning(self, "Hata", "Lütfen bir kuaför seçiniz.")
                return
            
            # 2. Veritabanına ekle ve dönen sonucu kontrol et
            sonuc = self.db_manager.randevu_ekle(randevu_verisi)

            if sonuc == "DOLU":  #eğer veritabanında dolu döndürürse kullanıcıya sor
...
if __name__ == '__main__':
    ...
    db_manager = DataBaseManager()  #db manager ı başlat
